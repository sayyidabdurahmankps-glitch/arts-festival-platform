...
function useTeamComparisonData() {
  ...
  const fetchStats = async () => {
    const { data, error } = await supabase.from('team_comparison_stats').select('*');
    if (error) {
  ...
  useEffect(() => {
    ...
    const resultsChannel = supabase.channel('comparison-results-sync')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'results', filter: 'status=eq.approved' }, fetchStats)
      .subscribe();
...
function CustomTeamSelect({ 
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      >
        <span className={`font-black text-sm sm:text-base md:text-lg uppercase tracking-wide truncate pr-4 ${!selectedOption ? "text-zinc-600" : "text-white"}`}>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {disabled ? (
            ...
            <ChevronDown className={`text-zinc-400 w-4 h-4 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
          )}
        </div>
      </div>
        ...
        {isOpen && !disabled && (
          ...
          >
            <div className="max-h-[240px] sm:max-h-[280px] overflow-y-auto dropdown-scrollbar p-1">
              {options.length === 0 ? (
                <div className="p-4 sm:p-5 text-center text-zinc-600 text-[10px] sm:text-xs font-black uppercase tracking-widest">No opponents available</div>
              ) : (
                options.map((opt) => {
                  const isSelected = value === opt.team;
                  const categoryName = ["Indigo", "Violet"].includes(opt.team) ? "HIFZ CATEGORY" : "GENERAL CATEGORY";
                    ...
                    >
                      <div className="flex items-center gap-3 sm:gap-4 truncate">
                        ...
                        <div className="flex flex-col truncate">
                          <span className={`font-black uppercase tracking-wide text-sm sm:text-base leading-tight truncate transition-colors ${isSelected ? 'text-white' : 'text-zinc-400 group-hover:text-zinc-200'}`}>
                            {opt.team}
                          </span>
                      ...
                      {isSelected && <Check className="w-4 h-4 sm:w-5 sm:h-5 animate-in zoom-in shrink-0 ml-2" style={{ color: opt.color }} />}
                    </div>
                  );
                })
              )}
...
export default function TeamComparison() {
  ...
  const teamA = useMemo(() => allStats.find((t) => t.team === teamAName), [teamAName, allStats]);
  const teamB = useMemo(() => allStats.find((t) => t.team === teamBName), [teamBName, allStats]);

  const availableForB = useMemo(() => {
    return allStats.filter(
      (t) => t.team !== teamAName && (!teamA || getTeamGroup(t.team) === getTeamGroup(teamA.team))
    );
  ...
  const comparisonData = useMemo(() => {
    if (!teamA || !teamB) return [];
    return [
      { name: teamA.team, points: teamA.total_points || 0, color: teamA.color || "#6366f1" },
      { name: teamB.team, points: teamB.total_points || 0, color: teamB.color || "#a855f7" },
    ];
  }, [teamA, teamB]);
  ...
  const analysisText = useMemo(() => {
    ...
    if (ptsA > ptsB) return <><span style={{ color: teamA.color }}>{teamA.team}</span> leads by a crushing {ptsA - ptsB} points.</>;
    if (ptsB > ptsA) return <><span style={{ color: teamB.color }}>{teamB.team}</span> leads by a crushing {ptsB - ptsA} points.</>;
  ...
  if (loading) {
    return (
      <div className="bg-[#050505] p-6 sm:p-12 rounded-[2rem] md:rounded-[3.5rem] shadow-2xl border border-white/5 flex flex-col items-center justify-center min-h-[400px] sm:min-h-[500px]">
        ...
        <p className="text-zinc-500 font-black uppercase tracking-widest text-[10px] sm:text-xs">Loading Combat Intelligence...</p>
      </div>
    );
  }
    ...
    <div className="bg-[#050505] p-5 sm:p-8 md:p-12 rounded-[2rem] md:rounded-[3.5rem] shadow-2xl border border-white/5 relative overflow-visible group w-full max-w-full">
      ...
      {teamA && teamB && (
        <div
          ...
          style={{ background: `linear-gradient(90deg, ${teamA.color} 0%, transparent 40%, transparent 60%, ${teamB.color} 100%)` }}
        />
      )}
      ...
      <div className="flex flex-col md:grid md:grid-cols-[1fr_auto_1fr] items-center gap-4 sm:gap-6 mb-8 md:mb-12 relative z-50">
        ...
        <div className="w-full">
          ...
          <CustomTeamSelect 
            value={teamAName}
            onChange={(val) => { setTeamAName(val); setTeamBName(""); }}
            options={allStats}
        ...
        >
            ...
            <table className="w-full text-left border-collapse min-w-[650px] md:min-w-[700px]">
              <thead>
                <tr className="border-b border-white/10 text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 bg-zinc-950/80">
                  <th className="py-4 md:py-5 px-4 md:px-6 w-12 md:w-16 text-center">Rank</th>
                  <th className="py-4 md:py-5 px-4 md:px-6">Faction</th>
                  <th className="py-4 md:py-5 px-3 md:px-4 text-center">1st (G)</th>
                  <th className="py-4 md:py-5 px-3 md:px-4 text-center">2nd (S)</th>
                  <th className="py-4 md:py-5 px-3 md:px-4 text-center">Grade A</th>
                  <th className="py-4 md:py-5 px-3 md:px-4 text-center">Grade B</th>
                  <th className="py-4 md:py-5 px-3 md:px-4 text-center">Extra Pts</th>
                  <th className="py-4 md:py-5 px-4 md:px-6 text-right text-indigo-400">Total Score</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5 text-white">
                {sortedTeams.map((team, index) => {
                  const isLeader = index === 0;
                  const extraPts = (team.bonus_points || 0) - (team.penalty_points || 0);
                    ...
                    >
                      <td className="py-3 md:py-4 px-4 md:px-6 text-center">
                        {isLeader ? (
                          ...
                          <span className="font-black text-zinc-500 text-sm">2</span>
                        )}
                      </td>
                      <td className="py-3 md:py-4 px-4 md:px-6 font-black uppercase tracking-wide flex items-center gap-2 md:gap-3 text-sm md:text-base">
                        <div className="w-2.5 h-2.5 md:w-3 md:h-3 shrink-0 rounded-full shadow-sm" style={{ backgroundColor: team.color }} />
                        <span className="truncate">{team.team}</span>
                      </td>
                      <td className="py-3 md:py-4 px-3 md:px-4 text-center font-bold tabular-nums text-zinc-300 text-sm">{team.gold || 0}</td>
                      <td className="py-3 md:py-4 px-3 md:px-4 text-center font-bold tabular-nums text-zinc-300 text-sm">{team.silver || 0}</td>
                      <td className="py-3 md:py-4 px-3 md:px-4 text-center font-bold tabular-nums text-zinc-300 text-sm">{team.grade_a || 0}</td>
                      <td className="py-3 md:py-4 px-3 md:px-4 text-center font-bold tabular-nums text-zinc-300 text-sm">{team.grade_b || 0}</td>
                      <td className={`py-3 md:py-4 px-3 md:px-4 text-center font-bold tabular-nums text-sm ${extraPts > 0 ? 'text-emerald-400' : extraPts < 0 ? 'text-red-400' : 'text-zinc-500'}`}>
          ...
          <div className="h-[250px] sm:h-[300px] md:h-[350px] bg-black/40 rounded-[1.5rem] md:rounded-3xl p-4 sm:p-6 md:p-8 border border-white/5 flex items-center justify-center relative backdrop-blur-xl">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={comparisonData} margin={{ top: 20, right: 10, left: -25, bottom: 0 }}>
